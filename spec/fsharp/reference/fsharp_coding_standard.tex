...
\paragraph{P2.3 Stop using ADO.NET for reads.  Use SqlHydra + \texttt{query \{ \}}.}
Every script repeats the pattern
\texttt{use cmd = new SqliteCommand(...); cmd.Parameters.AddWithValue(...); use rdr = cmd.ExecuteReader(); r.GetString~0; r.GetInt32~1}
30--40 times.  This is 2003 technology.  F\# has had its own
native SQL DSL since F\# 3.0 (\texttt{query \{ \}}); the 2026
idiomatic stack for typed access is
\textbf{SqlHydra}\footnote{\url{https://github.com/JordanMarr/SqlHydra}}
--- a code generator (not a type provider) recommended by the
\href{https://fsharp.org/guides/data-access/}{fsharp.org data
access guide}.

Workflow:

\begin{Verbatim}[fontsize=\small,frame=single]
# One-time, after seeding indus_codebook.db:
dotnet tool install --global SqlHydra.Cli
dotnet sqlhydra sqlite
# → writes IndusCodebook.fs (committed to the repo)
\end{Verbatim}

Then every read script becomes:

\begin{Verbatim}[fontsize=\small,frame=single]
#load "IndusCodebook.fs"
open IndusCodebook

let ctx = Sqlite.CreateContext(connStr)

let jarCommodity =
    query {
        for c in ctx.Commodity do
        where (c.code = "jar")
        select (c.code, c.label, c.description)
    } |> Seq.tryHead
\end{Verbatim}

\texttt{c.code} is a typed string, not \texttt{r.GetString 0}.
Rename a column $\to$ compile error.  No placeholder mismatches.
No \texttt{AddWithValue} type sniffing.  Reads like SQL.

Why SqlHydra and not a type provider: type providers generate
phantom types at compile time --- the professor cannot see them.
SqlHydra generates \texttt{IndusCodebook.fs} on disk.  The
professor opens it, reads the record declarations, sees the
schema in F\#.  The schema becomes \emph{documentation}, not
compiler magic.

Why not replace ADO.NET everywhere: SqlHydra reads an existing
schema.  The seed script (\texttt{indus\_seed\_codebook.fsx})
creates tables --- DDL work --- and stays on
\texttt{Microsoft.Data.Sqlite} with raw
\texttt{CREATE TABLE} statements.  After the seed runs,
regenerate \texttt{IndusCodebook.fs} with \texttt{dotnet
sqlhydra sqlite} and every consumer script uses
\texttt{query \{ \}}.

Scope of this P2.3 change: \texttt{indus\_decoder.fsx},
\texttt{indus\_tn\_decoder.fsx}, \texttt{indus\_lssc.fsx}, the
radiometric-ingestion block in
\texttt{indus\_ingest\_corpus.fsx}.  The seed and the DDL blocks
stay on ADO.NET.
