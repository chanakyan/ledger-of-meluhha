...
\paragraph{P2.3 Consolidate the ADO.NET boilerplate.}
Every script repeats the pattern
\texttt{use cmd = new SqliteCommand(...); cmd.Parameters.AddWithValue(...); cmd.ExecuteNonQuery() |> ignore}
30--40 times.  Collapse into one helper per script (or per
shared module if you introduce one):

\begin{Verbatim}[fontsize=\small,frame=single]
let exec (con: SqliteConnection) (tx: SqliteTransaction)
         (sql: string) (parameters: (string * obj) list) =
    use cmd = new SqliteCommand(sql, con, tx)
    for (k, v) in parameters do
        cmd.Parameters.AddWithValue(k, v) |> ignore
    cmd.ExecuteNonQuery() |> ignore
\end{Verbatim}

The payoff is not abstraction --- it is noise removal.  The
meaningful SQL stops being buried.
...
Do not introduce type providers, computation expressions,
active patterns, or custom operators.  The professor reads the
code.  She does not read Wlaschin.
